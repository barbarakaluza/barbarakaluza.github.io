...
const RecipeForm = ({ recipe, onClose, onAddRecipe }) => {
  // Inicjalizujemy stan formularza z danymi z 'recipe'
  ...
  useEffect(() => {
    if (recipe) {
      console.log("Załadowanie danych do formularza: ", recipe); // Logowanie danych przepisu
      setFormData({
  ...
  }, [recipe]);  // Tylko załaduj dane, gdy 'recipe' się zmieni
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Przekazujemy dane do rodzica
    onAddRecipe({
      id: recipe.id,  // Upewnij się, że id przepisu jest zachowane
